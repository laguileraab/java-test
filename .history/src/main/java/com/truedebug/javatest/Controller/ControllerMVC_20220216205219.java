...
public class ControllerMVC {
    ...
    public String gesMenu(@ModelAttribute Menu menu, BindingResult result,Model model){
        ...
        ///Enviar datos del POST para Modificar/Eliminar un menu de la BD
        @RequestMapping(value = "/ges/{uuid}")
        public String gesMenu(@ModelAttribute Menu menu, Model model,@PathVariable UUID uuid){
    
            //Necesario para enviar los datos a /menu/ges/{uuid}
            model.addAttribute("person",person);
            Menu menu = menuService.getMenuById(uuid);
            ///
            if(!isAutenticated){
                return "index.html";
            }
        ...
        public String delMenu(@ModelAttribute Menu menu, Model model,@PathVariable UUID uuid){
            ...
            //Se muestra el día de la semana en español
            model.addAttribute("date", new Day().valueOf(Calendar.DAY_OF_WEEK));
