...
import com.truedebug.Utils.Compare;
...
public class ControllerMVC {
    ...
    public ModelAndView listMenu(){
        ...
        LocalDate date = Compare.compareDate(person.getDate());
        ...
        menus.forEach((m)->{
            ...
            String storedDate2 = simpleDateFormat2.format(m.getDate());
            LocalDate date2 = LocalDate.parse(storedDate2);
            if(java.time.LocalDate.now().isEqual(date2)){
                menu = m;
            }
        });
    ...
    public String listMenu(@ModelAttribute String recomendations, @PathVariable int option, Model model){
        //ModelAndView model = new ModelAndView();
        if(!isAutenticated){
            //model.setViewName("index.html");
            //return model;
